...
def handle_dialog(request, response, user_storage, database):
    ...
    if database.get_session(request.user_id, 'status_action')[
        ...
        message = Message(username=user[1], message=request.command, recipient=user[2], status=1)
    ...
    if 'напиши' in input_message and len(request.command.split(' ')) == 2 and \
        ...
        if User.query.filter_by(username=request.command.split(' ')[1]).first():
            ...
            user_storage = {
                'suggests': ['Отмена', 'Друзья', 'Группы', 'Найти', 'Помощь', 'Главная']}
            database.update_status_system(request.user_id, 'sending_letter', 'status_action')
            database.update_status_system(request.user_id, request.command.split(' ')[1],
                                          'recipient_name')
        else:
